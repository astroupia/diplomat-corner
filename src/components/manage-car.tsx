...
const ManageCar: React.FC<ManageCarProps> = ({ initialData, isEditMode = false }) => {
  ...
  const [formData, setFormData] = useState<CarFormData>({
    ...
    advertisementType: initialData?.advertisementType as "Rent" | "Sale" || "Sale",
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === "year" || name === "mileage" || name === "price" || name === "servicePrice" 
        ? Number(value) 
        : value,
    }));
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const endpoint = isEditMode ? `/api/cars/${initialData?._id}` : '/api/cars';
      ...
      if (result.success) {
        setSubmitResult({ 
          success: true, 
          message: isEditMode ? "Car updated successfully!" : "Car saved successfully!" 
        });
        ...
        setSubmitResult({ success: false, message: result.error || "Failed to save car" });
      }
    } catch (error) {
      setSubmitResult({
        success: false,
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 rounded-3xl p-4 lg:p-6">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap space-x-2">
                      {["Automatic", "Manual"].map((option) => (
                        ...
                        >
                          {formData.transmission === option ? <CheckCircle size={16} /> : <Circle size={16} />}
                          <span>{option}</span>
                        </button>
                      ))}
                    </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap space-x-2">
                    {["Gasoline", "Diesel", "Electric"].map((option) => (
                      ...
                      >
                        {formData.fuel === option ? <CheckCircle size={16} /> : <Circle size={16} />}
                        <span>{option}</span>
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap space-x-2">
                    {["Truck", "SUV", "Sedan", "Hatchback", "Minivan"].map((option) => (
                      ...
                      >
                        {formData.bodyType === option ? <CheckCircle size={16} /> : <Circle size={16} />}
                        <span>{option}</span>
                      </button>
                    ))}
                  </div>
              ...
              <div className="col-span-12 lg:col-span-4 space-y-6 border-2 border-primary p-4 lg:p-6 rounded-3xl shadow-md overflow-y-auto max-h-[calc(100vh-200px)]">
                <div className="space-y-2">
                  <label className="block text-sm font-semibold text-primary">Car Image</label>
                    ...
                    {imagePreview ? (
                      <>
                        ...
                        <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                          ...
                          <p className="mt-4 text-sm text-white">Click to change image</p>
                        </div>
                      </>
                    ) : (
                      <>
                        <Upload size={40} className="text-primary" />
                        <p className="mt-4 text-sm text-primary">Upload car image</p>
                      </>
                    )}
                  </div>
                  <input
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-semibold text-primary">Payment Receipt</label>
                  <div className="h-40 flex flex-col items-center justify-center border-dashed border-2 border-primary rounded-lg relative">
                    {receiptPreview ? (
                      <>
                        <img
                        ...
                        <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                          <Upload size={40} className="text-white" />
                          <p className="mt-4 text-sm text-white">Click to change receipt</p>
                        </div>
                      </>
                    ) : (
                      <>
                        <Upload size={40} className="text-primary" />
                        <p className="mt-4 text-sm text-primary">Upload payment receipt</p>
                      </>
                    )}
                  </div>
                  <input
                ...
                <div className="space-y-2">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-primary">Service Price *</label>
                    <input
                      type="number"
                      name="servicePrice"
                  ...
                  <div className="flex space-x-4">
                    {(["ETB", "USD"] as const).map((option) => (
                      ...
                      >
                        {formData.currency === option ? <CheckCircle size={16} /> : <Circle size={16} />}
                        <span>{option}</span>
                      </button>
                    ))}
                  </div>
